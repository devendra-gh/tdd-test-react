...
export async function init(props: any) {
  ...
  props.actions.tableColumns.update([
    {
      id: 'name',
      title: props.i18n('individual_owner_label_name'),
      sortable: true,
...
export  function call_f2_onChange(props: any) {
    ...
    if (value) {
      ...
      props.actions.divisionDisabled.update(true);
...
export async function f13_onSort(props: any) {
}
export async function f14_onChange(props: any) {
}
export async function f15_onSubmit(props: any) {
	  props.actions.loading.update(true);
  let activityCodesArr: any[] = [];
  props.basket.map((item: any) => {
    activityCodesArr.push({ ActivityCode: item.id });
  });
  props.actions.activityCodes.update(activityCodesArr);
  props.history.push('/choose-economic-name');
}
export  function call_f16_onRemove(props: any) {
    ...
    const selectedActivities = updatedActivities
      .filter((item: any) => item.selected)
      .map((obj: any) => obj._id);
    props.actions.selectedActivities.update(selectedActivities);
...
export async function f18_btnBackClick(props: any, formValues: any) {
	  //props.fetch();
...
export async function f19_btnSubmitClick(props: any, formValues: any) {
	  //props.fetch();
  const { fetch, bpm } = props;
  ...
  let res: any;
  props.actions.loading.update(true);
  if (props.skipEconomicName) {
    res = await props.bpm.sendMessage({
      businessKey: props.businessKey,
    ...
    res = await props.bpm.sendMessage({
      businessKey: props.businessKey,
      messageName: 'onNextToChooseEconomicName',
      variables: {
        selectedActivities: JSON.stringify(props.basket),
        activityCodes: JSON.stringify(activityCodes),
      },
...
export async function f20_btnCancelClick(props: any, formValues: any) {
	   // props.actions();
  props.actions.isCancelModalOpen.update(true);
}
export async function f21_btnSubmitDisabled(props: any, formValues: any) {
	  return props.chooseActivitiesBtnDisabled;
}
export async function f22_primaryButton_onClick(props: any) {
	props.actions.isModalOpen.update(false);
}
export async function f23_onClose(props: any) {
	props.actions.isModalOpen.update(false);
}
export async function f24_primaryButton_onClick(props: any) {
	  // props.actions();
...
export async function f25_secondaryButton_onClick(props: any) {
	  // props.actions();
  props.actions.isCancelModalOpen.update(false);
}
export async function f26_onClose(props: any) {
	  props.actions.isCancelModalOpen.update(false);
}
