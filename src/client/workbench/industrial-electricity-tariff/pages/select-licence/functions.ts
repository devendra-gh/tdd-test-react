...
import {getPaginatedFilteredLicenceListItems, getFilteredBySearchLicences, paginateLicenceListItems} from '../../sharedFunctions/utils';
...
export async function init(props: any) {
  ...
  let licenceList = await fetchLicenceList(
    props,
    props.user.IDN,
    false,
  );
  licenceList = licenceList
    ...
    .reduce(
      ({ seenLicences, uniqueLicences }: IVariables, { tradeLicenseNumber, ...rest }: IVariables) => {
...
export  function call_f2_onSearch(props: any) {
    ...
    const licenceList = props.licenceList;
    const filteredLicenceList = getFilteredBySearchLicences(
      licenceList,
      value,
    );
    ...
    console.log('the provided paginated licence list', paginatedFilteredLicences);
...
export  function call_f8_onSearch(props: any) {
    ...
    props.actions.licenceSearchValue.update(value);
    const licenceList = props.licenceList;
    const filteredLicences = getFilteredBySearchLicences(
      licenceList,
      value,
    );
