...
const getAwards = async (props: any, apTransactionNumber: string) => {
  ...
  try {
    const token = await getDEDToken(props);
    const response = await props.fetch(
      `/api/proxy/ms-call/gateway/TammJourneyDed/1.0/industrialLicense/industrialLocalContentCertificate/previewQuotations?token=${token}`,
      'post',
      {
        body: {
          icvNumber: apTransactionNumber,
          withDocuments: true,
        },
        headers: {
