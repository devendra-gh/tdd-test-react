...
const init = async (props: IVariables) => {
  ...
  // @ts-ignore
  const currentFormState = JSON.parse(localStorage.getItem('formState'));
  if (currentFormState && currentFormState.licenceType) {
    currentFormState.licenceType = 'tamm';
    localStorage.setItem('formState', JSON.stringify(currentFormState));
  }
