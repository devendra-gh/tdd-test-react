...
describe('validateRule', () => {
  ...
  /*
    it('it return error message when invalid date if date', () => {
      
      const result = validateRule('date', 'a12');
      expect(result).toEqual({ message: 'form.error.date' });
    });

    it('it return error message when invalid date if date before', () => {
      
      const result = validateRule(
          { rule: 'date', before: '12-12-2022', message: 'error' },
          '12-12-2023',
        );
      expect(result).toEqual({ message: 'error' });
    });

    it('it return error message when invalid date if date after', () => {
      
      const result = validateRule(
          { rule: 'date', after: '12-12-2022', message: 'error' },
          '12-12-2021',
        );
      expect(result).toEqual({ message: 'error' });
    });
