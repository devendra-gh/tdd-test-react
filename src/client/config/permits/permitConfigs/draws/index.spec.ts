...
describe('On Change handlers', () => {
  it('should call the on change function with the right params', () => {
    ...
    const subFieldsWithFunctionValues = config.formFields.salesBanners.fields.reduce(
      (acc, field: any) => {
    ...
    const onChangeAirFunctionFields = config.formFields.prizes.fields[0].subFields.filter(
      field => typeof field.onChange === 'function',
    );
...
describe('permitAdPapers', () => {
  it('should call the disabled function', () => {
    const customDisabledDateFunctionFields = config.formFields.prizes.fields[0].subFields.filter(
      field => typeof field.customDisabledDate === 'function',
    );
    customDisabledDateFunctionFields.forEach(field =>
      // @ts-ignore
      field.customDisabledDate('2020-02-02', {
        permitDetails: { startDate: '2020-02-02', endDate: '2020-12-02' },
      }),
    );
    expect(validateShowDate).toHaveBeenCalled();
  });
});
// eslint-disable-next-line no-console
console.log = () => {};
