...
describe('Form Submit Details', () => {
  ...
  it('should return form submit details with empty applicantContact name', () => {
    ...
    const dummyServiceDetails = {
      permitDetails: { promotionType: 'Scratch and Win' },
      applicantContact: {
        name: '',
        phone: '987654',
      },
...
describe('On Change handlers', () => {
  it('should call the on change function with the right params', () => {
    ...
    config.formFields.documents.fields.forEach(
      (fieldWithFunctionValue: any) => {
        // @ts-ignore

        fieldWithFunctionValue.conditionalBehaviour({
          permitDetails: { promotionType: 'Scratch and Win' },
        });

        expect(fieldWithFunctionValue.conditionalBehaviour).toBeInstanceOf(
          Function,
        );
      },
    );
