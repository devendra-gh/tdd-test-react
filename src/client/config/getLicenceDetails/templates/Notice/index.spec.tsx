...
describe('config/templates/Notice', () => {
  ...
  it('should render button link', () => {
    ...
    setTimeout(() => {
      fireEvent.click(getByLabelText('button-primary1'));

      expect(getByLabelText('button-primary1')).toBeInTheDocument();
    }, 5000);
  });
  ...
  it('should handle button onClick', () => {
    ...
    setTimeout(() => {
      fireEvent.click(getByLabelText('button-primary2'));
  ...
  it('should render without step tracker button click with null value', async () => {
    props.currentStep = false;
    const { getByLabelText } = render(
      <MemoryRouter>
        <ViewportProvider>
    ...
    const button = await waitForElement(() =>
      getByLabelText('button-primary1'),
    );
    fireEvent.click(button);
  });
});
